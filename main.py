...
def load_json_file(file_path):
    ...
    for sentence in data:
        ...
        for line in sentence:
            ...
            tag.append(line["Tag"])
        r.append({"Head word": head, "Function word": function,
...
def format_prompt(examples, input_sentence, opt):
    ...
    if opt == 1:
        ...
        prompt = "Perform Named Entity Recognition (NER) on Japanese National Pention Law with the following tags:\n"
        prompt += "S - Subject, E - Effectuation, R - Requisite; O for outside; I for inside (e.g., I-E) and E for ending (e.g., E-E).\n\n"
        prompt += "Examples:\n"
...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained(MODEL_NAME, token=access_token, device_map="auto", torch_dtype=torch.float16)
    # model.to(DEVICE)
    ...
    for file in test_name:
        ...
        for i in tqdm(range(len(test_samples))):
            ...
            output = perform_ner(   
                model, tokenizer, examples[5], test_samples[i], opt=prompt_opt)
